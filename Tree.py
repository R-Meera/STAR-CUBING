...
class Tree(object):
    ...
    def generate(self, df):
        numDim = len(df.columns) - 1
        for i in range(len(df)):
            for j in range(numDim - 1):
                if self.head.get_left() == None:
                    new_left = Node(data=df.iloc[i, j], count=df.iloc[i, numDim], parent=self.head, depth=j)
                    ...
                    if self.head.get_right() == None and self.head.get_data() != df.iloc[i, j]:
                        new_right = Node(data=df.iloc[i, j], count=df.iloc[i, numDim], parent=self.head.get_parent(),
                                         depth=j)
                        self.head.set_right(new_right)
                        self.head = self.head.get_right()
                    elif self.head.get_data() == df.iloc[i, j]:
                        self.head.set_count(self.head.get_count() + df.iloc[i, numDim])
            self.head = self.root
